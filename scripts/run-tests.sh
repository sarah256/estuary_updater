...
if [ "$1" != "" ]; then
    ...
    # of the default
    docker-compose -f docker/docker-compose-tests.yml run --rm estuary_updater_tests sh -c "while ! nc -z -w 2 neo4j 7687; do sleep 1; done; $*"
else
    docker-compose -f docker/docker-compose-tests.yml run --rm estuary_updater_tests
fi
RESULT=$?
